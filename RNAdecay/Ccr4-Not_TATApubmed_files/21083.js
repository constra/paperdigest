...
Portal.Portlet.RelatedDataLinks = Portal.Portlet.NCBIPageSection.extend({
    ...
    'SendLink': function(e, target, name){ 
        this.send.DbChanged({'db' : this.getValue("rdDatabase")});
        this.send.Cmd({'cmd' : 'link'});
        this.send.LinkName({'linkname' : this.getValue("rdLinkOption")});
        Portal.requestSubmit(); 
    },
...
jQuery.widget("ui.ncbiyoutube", {

    options: {
        videoId: "",
        allowFullScreen: "true",
        related: "0",  // 0 off, 1 on
        width: "265",
        height: "174"
    },
    ...
    _create: function () {
        var that = this;
        this.element.click( function(){ that._replaceImage(); });
    },
    ...
    _replaceImage: function(){
        ...
        this.vid = "jiglog" + this.id;
        this._addPlayer();
        this._addReadyListener();
        this._addReadyTrigger();
        this._createStateObj();
        this._watchForActions();    
    },
    
    destroy : function(){
        this._removeActionWatcher();
        this._removePlayer();
    ...
    _addPlayer: function () {
        ...
        if (jQuery.browser.msie) {
            var playerCode = '<OBJECT id=[pid] classid=clsid:D27CDB6E-AE6D-11cf-96B8-444553540000 width=[wid] height=[hei]><PARAM NAME="Movie" VALUE="http://www.youtube.com/v/[vid]&amp;border=0&amp;enablejsapi=1&amp;playerapiid=[pid]&amp;fs=1&amp;rel=[rel]&amp;autoplay=1"><PARAM NAME="Src" VALUE="http://www.youtube.com/v/[vid]&amp;border=0&amp;enablejsapi=1&amp;playerapiid=[pid]&fs=1&amp;autoplay=1"><PARAM NAME="AllowScriptAccess" VALUE="always"><PARAM NAME="AutoPlay" VALUE="1"><PARAM NAME="AllowFullScreen" VALUE="[ful]"></OBJECT>';
        } else {
            playerCode = '<object data="http://www.youtube.com/v/[vid]&amp;amp;border=0&amp;amp;enablejsapi=1&amp;amp;playerapiid=[pid]&amp;amp;fs=1&amp;rel=[rel]&amp;amp;autoplay=1" id="[pid]" type="application/x-shockwave-flash" height="[hei]" width="[wid]"><param value="always" name="allowScriptAccess"><param value="[ful]" name="AllowFullScreen"><param name="wmode" value="opaque" /><param name="autoplay" value="1" /></object>';       
        }

        playerCode = playerCode.replace(/\[wid\]/g, this.options.width).replace(/\[hei\]/g, this.options.height).replace(/\[ful\]/g, this.options.allowFullScreen).replace(/\[pid\]/g, this.vid).replace(/\[vid\]/g, this.options.videoId).replace(/\[rel\]/g, this.options.related);
        jQuery(playerCode).appendTo(this.element);
    ...
    _addReadyTrigger: function () {
        var ref = this;        
        if (!jQuery.ui.ncbiyoutube._isTriggerSet) {
            jQuery.ui.ncbiyoutube._isTriggerSet = true;
            window["onYouTubePlayerReady"] = function (id) {
                jQuery("#" + id.substring(6)).trigger("ready");
            }
        }
    },

    _addReadyListener: function () {
        var ref = this;
        this.element.bind("ready", function () {
            ref._lockedAndLoaded();
        });
    },
    ...
    _addPlayerListeners: function () {
        ...
        var fncStateChange = this.id + "_statechange"
        var fncQualityChange = this.id + "_qualitychange"
        var fncError = this.id + "_error"

        window[fncStateChange] = function (state) {
            ref._playerStateChanged(state);
        }
        window[fncQualityChange] = function (state) {
            ref._playerPlaybackQualityChange(state);
        }
        window[fncError] = function (state) {
            ref._playerError(state);
        }

        this._videoObj.addEventListener("onStateChange", fncStateChange);
        this._videoObj.addEventListener("onPlaybackQualityChange", fncQualityChange);
        this._videoObj.addEventListener("onError", fncError);

    },
    
    _removePlayerListeners: function(){
    
        var fncStateChange = this.id + "_statechange"
        var fncQualityChange = this.id + "_qualitychange"
        var fncError = this.id + "_error"

        delete window[fncStateChange];
        delete window[fncQualityChange];
        delete window[fncError];
        
    },
    

    _lockedAndLoaded: function () {
        this._videoObj = jQuery("#" + this.vid)[0];
        this._addPlayerListeners();
    },
    
    getDuration : function(){
        return this._videoObj?this._videoObj.getDuration():null;
    },
    
    getVideoBytesTotal : function(){
        return this._videoObj?this._videoObj.getVideoBytesTotal():null;
    },
    
    getCurrentTime : function(){
        return this._videoObj?this._videoObj.getCurrentTime():null;
    },
    
    getVideoUrl : function(){
        return this._videoObj?this._videoObj.getVideoUrl():null;
    },

    isMuted : function(){
        return this._videoObj?this._videoObj.isMuted():null;
    },

    getVolume : function(){
        return this._videoObj?this._videoObj.getVolume():null;
    },

    getVideoStartBytes : function(){
        return this._videoObj?this._videoObj.getVideoStartBytes():null;
    },

    getPlaybackQuality : function(){
        return this._videoObj?this._videoObj.getPlaybackQuality():null;
    },
    
    _playerStateChanged: function (state) {
        switch (state){
            case -1: //unstarted
                break;
            case 0: // Media Ended
                this.statState("Ended");
                break;
            case 1: // Playing
                this.statState("Playing");
                break;
            case 2: // Paused
                this.statState("Paused");
                break;
            case 3: // Buffering               
                break;
            case 5: // Ready
    ...
    _playerPlaybackQualityChange : function(){
        this.statState("qualityChanged");
    },

    _playerError : function(){
        this.statState("videoError");
    },
    
    _createStateObj : function(){
        this.stateObj = {
            volume : null,
            time : null
        }
    ...
    _watchForActions : function(){
        var ref = this;
        this.timer = window.setInterval( function(){ ref._watchValues(); } , 1000 );
        
    },
    
    _watchValues : function(){
    
        var currTime = this.getCurrentTime();
        if(currTime<0)currTime=0;
        var currVol = this.getVolume();
        
        var prevTime = this.stateObj.time;
        var prevVol = this.stateObj.volume;
        
        var rCurrTime = Math.round(currTime);
        var rPrevTime = Math.round(prevTime);
        
        if(prevVol===null){
            this.stateObj.volume = currVol;
        }
        else if(currVol!==prevVol){
            this.statState("volumeChanged", { previousVolume : prevVol } );
            this.stateObj.volume = currVol;
        }
        
        if(prevTime!==null){
            if(rCurrTime<rPrevTime){
                this.statState("scanBackward", { previousTime : prevTime } );
            }
            else if(rCurrTime>rPrevTime+2){
                this.statState("scanForward", { previousTime : prevTime } );
            }
        }
        this.stateObj.time = currTime;
    },
    
    _removeActionWatcher : function(){
        if(this.timer)window.clearInterval(this.timer);
    },

    statState : function( action, additionalData ){

        var statObj = {
            jsEvent : "youtubeplayer",
            videoId : this.options.videoId,
            videoAction : action,
            currentTime : this.getCurrentTime(),
            isMuted : this.isMuted(),
            volume : this.getVolume(),
            videoStartBytes : this.getVideoStartBytes(),
            playbackQuality : this.getPlaybackQuality()
        }
        
        for(var prop in additionalData){
            statObj[prop] = additionalData[prop];
        }

        var ref = this.element.attr("ref");
        if(ref){
            var parts = ref.split("&");
            for(var i=0; i<parts.length; i++){
                var data = parts[i].split("=");
                statObj[data[0]] = data[1];
            }            
        }
               
        if( typeof ncbi !== "undefined" && ncbi.sg ){
            ncbi.sg.ping( statObj );
        }

    }
    
});
jQuery.ui.ncbiyoutube._isTriggerSet = false;
Portal.Portlet.Discovery_YouTube = Portal.Portlet.NCBIPageSection.extend({
  
   init: function(path, name, notifier) {
        console.info("Created Discovery_YouTube");
		this.base(path, name, notifier);		
		that = this;
	},
...
Portal.Portlet.Pubmed_Discovery_YouTube = Portal.Portlet.Discovery_YouTube.extend({
  
   init: function(path, name, notifier) {
        console.info("Created Pubmed_Discovery_YouTube");
		this.base(path, name, notifier);		
		that = this;
	},
	
	"getPortletPath" : function(){	
	    return (this.realname + ".Discovery_YouTube.NCBIPageSection");
	}
...
jQuery(function(){
    if (typeof imageStripObj !== 'undefined'){
        ...
        //temporarily for docsum ...
        //jQuery("#pubmed_imagesdb_sensor_toggler").click(imageStripObj.showImageStrip);
    }
